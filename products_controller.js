module.exports = {
    create: (req, res) => {
        ...
        dbInstance.create_product([name, description, price, image_url]).then(() => res.sendStatus(200))
            .catch(err => {
                res.status(500).send({ errorMessage: "Unable to Create Product" })
                console.log(err)
            })
    },
    getOne: (req, res) => {
        ...
        db.read_product(id).then(product => res.status(200).send(product))
            .catch(err => {
                res.status(500).send({ errorMessage: "Unable to retrieve product" })
                console.log(err)
            })
    },
    getAll: (req, res) => {
        ...
        dbInstance.read_products().then(products => res.status(200).send(products))
            .catch(err => {
                res.status(500).send({ errorMessage: "Unable to retrieve all products" })
                console.log(err)
            })
    },
    update: (req, res) => {
    ...
    delete: (req, res) => {
        ...
        dbInstance.delete_product(id).then(() => res.sendStatus(200))
            .catch(err => {
                res.status(500).send({ errorMessage: "Unable to delete product" })
                console.log(err)
            })
    }
}